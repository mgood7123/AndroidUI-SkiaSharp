...
if (!$SKIA_SHARP_PROJECT_BUILD_NUMBER.Equals($True) -and !$SKIA_SHARP_PROJECT_BUILD_NUMBER.Equals($False)) {
	...
	# externals-windows
	# externals-uwp
	dotnet cake --target=externals-windows --buildall=true --buildnumber=$SKIA_SHARP_PROJECT_BUILD_NUMBER --configuration=Debug --packall=false --solutionType=net6 --arch=x64
}