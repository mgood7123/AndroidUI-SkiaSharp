    ...
    .Does(() =>
{
    ...
    void Build(string arch, string skiaArch, string dir)
    {
        ...
        GnNinjaCAPI($"{VARIANT}/{arch}", "libSkiaSharp",
            $"target_os='win' " +
            $"target_cpu='{skiaArch}' " +
            $"skia_enable_fontmgr_win_gdi=false " +
            $"skia_use_dng_sdk=true " +
        ...
        CopyFileToDirectory(SKIA_C_PATH.CombineWithFilePath($"out/{VARIANT}/{arch}/libSkiaSharp.dll"), outDir);
        CopyFileToDirectory(SKIA_C_PATH.CombineWithFilePath($"out/{VARIANT}/{arch}/libSkiaSharp.dll.pdb"), outDir);
    }
});
