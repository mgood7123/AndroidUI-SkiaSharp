	...
	public unsafe class SKCanvas : SKObject
	{
		...
		virtual public SKSurface Surface {
			get {
				...
				return surface == IntPtr.Zero ? null : new SKSurface (surface, false);
			}
		}
		...
		virtual public int SaveLayerAlpha (float alpha)
		{
			if (1.0f == alpha) {
				return SaveLayer (null);
			} else {
				using SKPaint tmpPaint = new ();
				tmpPaint.AlphaF = alpha;
				return SaveLayer (tmpPaint);
			}
		}

		virtual public int SaveLayerAlpha (SKRect bounds, byte alpha)
		{
			if (255 == alpha) {
				return SaveLayer (bounds, null);
			} else {
				using SKPaint tmpPaint = new ();
				tmpPaint.Alpha = alpha;
				return SaveLayer (bounds, tmpPaint);
			}
		}

		virtual public SKImageInfo Info {
			get {
				SKImageInfoNative cinfo;
				SkiaApi.sk_canvas_get_image_info (Handle, &cinfo);
				return SKImageInfoNative.ToManaged (ref cinfo);
			}
		}

		virtual public int SaveLayerAlpha (SKRect bounds, float alpha)
		{
			if (1.0f == alpha) {
				return SaveLayer (bounds, null);
			} else {
		...
		virtual public void ClipShader (SKShader shader, SKClipOperation operation = SKClipOperation.Intersect)
		{
			if (shader == null)
				throw new ArgumentNullException (nameof (shader));

			SkiaApi.sk_canvas_clip_shader_with_operation (Handle, shader.Handle, operation);
		}
