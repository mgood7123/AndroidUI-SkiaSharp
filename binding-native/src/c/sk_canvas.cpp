...
class SKIA_SHARP_SLUG_CANVAS : public SkNWayCanvas {
public:

    SKIA_SHARP_SLUG_CANVAS(int width, int height) : SkNWayCanvas::SkNWayCanvas(width, height) {}

    void call_draw_slug(const sktext::gpu::Slug* slug) {
        onDrawSlug(slug);
    }
};

void sk_canvas_draw_slug(sk_canvas_t* ccanvas, sk_slug_t* slug) {
    SkCanvas* canvas = AsCanvas(ccanvas);
    auto size = canvas->getBaseLayerSize();
    SKIA_SHARP_SLUG_CANVAS c(size.width(), size.height());
    c.addCanvas(canvas);
    c.call_draw_slug(AsSlug(slug));
    c.removeCanvas(canvas);
}
...
void sk_canvas_set_matrix44(sk_canvas_t* ccanvas, const sk_m44_t* cmatrix) {
...
void sk_canvas_get_total_matrix44(sk_canvas_t* ccanvas, sk_m44_t* cmatrix) {
    *AsM44(cmatrix) = AsCanvas(ccanvas)->getLocalToDevice();
}
...
void sk_canvas_concat44(sk_canvas_t* ccanvas, const sk_m44_t* cmatrix) {
...
void sk_canvas_draw_image(sk_canvas_t* ccanvas, const sk_image_t* cimage, float x, float y, const sk_sampling_options_t* sampling_options, const sk_paint_t* cpaint) {
    AsCanvas(ccanvas)->drawImage(AsImage(cimage), x, y, *AsSamplingOptions(sampling_options), AsPaint(cpaint));
}

void sk_canvas_draw_image_rect_with_constraint(sk_canvas_t* ccanvas, const sk_image_t* cimage, const sk_rect_t* csrcR, const sk_rect_t* cdstR, const sk_sampling_options_t* sampling_options, const sk_paint_t* cpaint, sk_src_rect_constraint_t constraint) {
    AsCanvas(ccanvas)->drawImageRect(AsImage(cimage), *AsRect(csrcR), *AsRect(cdstR), *AsSamplingOptions(sampling_options), AsPaint(cpaint), (SkCanvas::SrcRectConstraint)constraint);
}
...
void sk_canvas_draw_image_lattice(sk_canvas_t* ccanvas, const sk_image_t* image, const sk_lattice_t* lattice, const sk_rect_t* dst, sk_filter_mode_t filter, const sk_paint_t* paint) {
    AsCanvas(ccanvas)->drawImageLattice(AsImage(image), *AsLattice(lattice), *AsRect(dst), (SkFilterMode)filter, AsPaint(paint));
}

void sk_canvas_draw_image_nine(sk_canvas_t* ccanvas, const sk_image_t* image, const sk_irect_t* center, const sk_rect_t* dst, sk_filter_mode_t filter, const sk_paint_t* paint) {
    AsCanvas(ccanvas)->drawImageNine(AsImage(image), *AsIRect(center), *AsRect(dst), (SkFilterMode)filter, AsPaint(paint));
}
...
void sk_canvas_draw_atlas(sk_canvas_t* ccanvas, const sk_image_t* atlas, const sk_rsxform_t* xform, const sk_rect_t* tex, const sk_color_t* colors, int count, sk_blendmode_t mode, const sk_sampling_options_t* sampling_options, const sk_rect_t* cullRect, const sk_paint_t* paint) {
    AsCanvas(ccanvas)->drawAtlas(AsImage(atlas), AsRSXform(xform), AsRect(tex), colors, count, (SkBlendMode)mode, *AsSamplingOptions(sampling_options), AsRect(cullRect), AsPaint(paint));
}
