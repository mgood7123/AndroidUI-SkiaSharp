...
sk_shader_t* sk_picture_make_shader(sk_picture_t* src, sk_shader_tilemode_t tmx, sk_shader_tilemode_t tmy, sk_filter_mode_t filter_mode, const sk_matrix_t* localMatrix, const sk_rect_t* tile) {
    SkMatrix m;
    if (localMatrix) {
        m = AsMatrix(localMatrix);
    }
    return ToShader(AsPicture(src)->makeShader((SkTileMode)tmx, (SkTileMode)tmy, (SkFilterMode)filter_mode, localMatrix ? &m : nullptr, AsRect(tile)).release());
}
